...
const ListFoderScreen = () => {
  ...
  const paginatedChildren = foderChild?.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
  );
  const paginatedImages = foder.images?.slice(
    (currentPage - 1) * pageSize,
    currentPage * pageSize
  );
        ...
        <div>
              ...
              <Col className="gutter-row" span={3} key={item.name}>
                <div className="Hover-Box">
                  ...
                  <ContextMenu
                    ...
                    onFolderDeleted={handleGetFolder}
                  />
                </div>
              </Col>
            ))}
                ...
                <div className="Hover-Box">
                  <div className="poiter">
                    <div className="img-box">
                      ...
                      />
                      <Image
                        src={API.API_HOST + "/" + item.name}
                        preview={false}
                        onClick={() =>
                          navigate(
                            `/image?folder=${returnFoderSegment(
                              item.path
                            )}&index=${index}&path=${item.name}`,
                            { state: { index: index } }
                          )
                        }
                        style={{ width: "100%" }}
          ...
          <div>
            <Pagination
              ...
              pageSize={pageSize}
              total={
                (foder.children?.length || 0) + (foder.images?.length || 0)
              }
              onChange={(page, size) => {
                setCurrentPage(page);
                setPageSize(size);
